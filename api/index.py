...
# 🔴 yahan apna proxy ya direct domain daalna hoga
DOMAIN_URL = "https://xnxx-dl.vercel.app/"   # <- isko zaroor change karein
    ...
    async def search(self, query: str):
        ...
        for element in soup.select("div.thumb-block"):
            ...
            video = {
                ...
                "thumbnail": element.select_one(".thumb img")["src"] if element.select_one(".thumb img") else None,
                ...
                "duration": (
                    ...
                    if element.select_one(".metadata") else "N/A"
                ),
            }
...
@app.get("/")
async def handler(action: str = Query(None), query: str = Query(None), url: str = Query(None)):
    if not action:
