...
class Downloader {
  ...
  async search(query) {
    try {
      ...
      const link = encodeURIComponent(
        `https://www.xnxx.com/search/${encodeURIComponent(query)}/${randomPage}`
      );

      const { data: html } = await axios.get(
        `https://example.com/api/tools/web/html/v1?url=${link}`,
      ...
      $("div.thumb-block").each((index, element) => {
        const video = {
          ...
          link: `https://www.xnxx.com${$(element)
            .find(".thumb-under a")
            .attr("href")}`,
          ...
          duration:
            $(element).find(".metadata").text().trim().split("\n")[1] || "N/A",
        };
  ...
  async detail(url) {
    try {
      const { data: html } = await axios.get(
        `https://example.com/api/tools/web/html/v1?url=${encodeURIComponent(
          url
        )}`,
        { headers: this.headers }
      );
      ...
      const duration =
        $('meta[property="og:duration"]').attr("content") || "N/A";
    ...
    switch (action) {
      case "search":
        if (!query)
          return res
            .status(400)
            .json({ error: "Query is required for search" });
      ...
      case "detail":
        if (!url)
          return res
            .status(400)
            .json({ error: "URL is required for detail" });
